const AppDealership = {
      ...
      <div class="view-scroll-content">
        <div class="fleet-controls-bar" style="flex-wrap: wrap; gap: 8px; margin-bottom: var(--space-3);">
          <div>
            ...
            <span style="font-size: 0.76rem; color: var(--text-muted);">Свободно слотов: <strong>${s.garage.slots - s.trucks.length}</strong> из ${s.garage.slots}</span>
          </div>
        ...
        <div class="market-trucks-compact-grid">
          ${catalog.map(m => this.generateCompactModelCardHTML(m)).join('')}
        </div>
      </div>
    `;
  },

  // Минималистичная компактная карточка (для сетки по 2 в ряд)
  ...
  // Окно с подробной информацией о тягаче и кнопкой покупки
    ...
    const html = `
      <div style="display: flex; flex-direction: column; gap: var(--space-4);">
        ...
        <table class="spec-detail-table">
          <tr>
            <td style="color: var(--text-muted);">Мощность силового агрегата:</td>
            <td>${m.enginePowerHp} л.с.</td>
          </tr>
          <tr>
            <td style="color: var(--text-muted);">${isElectric ? 'Емкость тяговой батареи:' : 'Объем топливного бака:'}</td>
            <td>${m.fuelTankCapacityL} ${isElectric ? 'кВт⋅ч' : 'л'}</td>
          </tr>
          <tr>
            <td style="color: var(--text-muted);">Паспортный расход:</td>
            <td>${m.baseFuelConsumptionL100} ${isElectric ? 'кВт⋅ч / 100 км' : 'л / 100 км'}</td>
          </tr>
          <tr>
            <td style="color: var(--text-muted);">Индекс надежности узлов:</td>
            <td>${Math.round(m.durabilityRating * 100)}%</td>
          </tr>
          <tr>
            <td style="color: var(--text-muted);">Коэффициент цены обслуживания:</td>
            <td>${Math.round(m.serviceCostMultiplier * 100)}%</td>
          </tr>
          <tr>
            <td style="color: var(--text-muted);">Свободно слотов в гараже:</td>
            <td style="color: ${availableSlots > 0 ? 'var(--accent-green)' : 'var(--accent-red)'};">${availableSlots} из ${s.garage.slots}</td>
          </tr>
        </table>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--glass-border); padding-top: 12px; margin-top: 4px;">
          ...
          <button class="btn-glass primary" 
            ${!canBuy ? 'disabled' : ''} 
            onclick="AppDealership.buyModel('${m.modelId}')">
